...
class LlamaModel:
    def __init__(self, parameters: LLMUtils.LLMConfig):
        ...
        self.ctxParams.n_ctx = self.parameters.nCtx
        ...
        if self.modelPath is not None:
            try:
                self.model = llama_cpp.llama_load_model_from_file(
                    self.modelPath.encode("utf-8"), self.modelParams
                )
        ...
        if self.model:
            self.context = llama_cpp.llama_new_context_with_model(
                self.model, self.ctxParams
            )

        self.n_ctx = llama_cpp.llama_n_ctx(self.context)
        self.nVocab = (
            llama_cpp.llama_n_vocab(self.model)
            if self.model
            else self.warnAndExit("__init__", "No model was found")
        )
        self.nCtx = (
            llama_cpp.llama_n_ctx(self.context)
            if self.context
            else self.warnAndExit("__init__", "No context was found")
        )
        self.__init()
    ...
    def _init(self):
        ...
        self.scores: npt.NDArray[np.single] = np.ndarray(
            (self.nCtx, self.nVocab), dtype=np.single
        )
    ...
    def warnAndExit(self, function, errorMessage):
        raise RuntimeError(
            f"LLMCore: Error in function: '{function}'. Following error message was provided: '{errorMessage}'\n"
        )

    def tokenizeFull(
        self, input: str, bos: bool = False, special: bool = True
    ) -> List[int]:
        tokens = (llama_cpp.llama_token * self.nCtx)()
        newTokens = llama_cpp.llama_tokenize(
            model=self.model,
    ...
    def evaluate(self, tokens: List[int], batch: int):
        if batch > 0:
            for i in range(0, len(tokens), batch):
                ...
                nPast = min(
                    self.nCtx - len(nBatch), len(self.inputIds[: self.pastTokens])
                )
                nTokens = len(nBatch)
                evalCode = llama_cpp.llama_eval(
                    ctx=self.context,
                ...
                if evalCode != 0:
                    self.warnAndExit(
                        "evaluate",
                        f"Error occured during evaluation of {len(tokens)} tokens with batch"
                        f"size {batch}",
                    )
                ...
                self.scores[
                    self.pastTokens + offset : self.pastTokens + nTokens, :
                ].reshape(-1)[:] = llama_cpp.llama_get_logits(self.context)[
                    : rows * cols
                ]
    ...
    def sampleTokenWithModel(self):
        lastNTokensSize = (
            self.parameters.n_keep if self.parameters.n_keep != -1 else self.nCtx
        )
        ...
        if self.parameters.temperature == 0.0:
            id = llama_cpp.llama_sample_token_greedy(
                ctx=self.context, candidates=llama_cpp.ctypes.byref(candidates)
            )
    ...
    def generate(self, stream: bool = False) -> str:
        antiPrompts: List[str] = self.parameters.antiPrompt
        exitTokens: List[int] = [2, 32002, 32001]
        tempBytes = b""
        finalString = ""
        tokens: List[int] = []
        tokenizedPromptTokens: List[int] = (
            self.tokenizeFull(self.parameters.prompt)
        ...
        if len(tokenizedPromptTokens) >= self.parameters.nCtx:
            print(
                f"{tokenizedPromptTokens} tokens were requested to be processed, maximum is "
        ...
        incompleteFix: int = 0
        for t in self.generateTokens(tokens=tokenizedPromptTokens):
            if t == self.EOSToken or t in exitTokens:
                finalString = (
                    self.tokensToString(tokens=tokens)
                    if len(finalString) + 1 != len(tokens)
                    else finalString
                )
                break
            tokens.append(t)
        ...
        return (
            finalString
            if finalString != ""
            else tempBytes.decode("utf-8", errors="ignore")
        )

    def response(self, stream: bool = False) -> str:
        if not stream:
    ...
    def loadPrompt(self, path: str = None, prompt: str = None, type: str = None):
        ...
        if path is not None:
            with open(path) as f:
                self.parameters.prompt = " " + (f.read()).replace(
                    "{llmName}", self.parameters.modelName
                )
    ...
    class LLamaOrig:
        def __init__(self, params):
            ...
            self.llama = Llama(
            model_path=self.parameters.modelPath,
    ...
    def response(self, stream=False):  
        res = str(
            self.llama(
                ...
                max_tokens=self.parameters.n_predict,
                mirostat_mode=2,
