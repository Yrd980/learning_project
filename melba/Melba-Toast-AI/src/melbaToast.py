...
class Melba:
    ...
    def prompt(self, person: str, message: str) -> str:
        ...
        finalPrompt = (
            (
                f"{self.llm.systemPromptPrefix} {systemprompt.replace('[personality]', personality)}\n"
                f"{self.llm.systemPromptSplitter}\n"
                f"{pastconversation}\n" + self.convoStyle
            )
            .replace("[context]", context)
            .replace("{llmName}", self.llmConfig.modelName)
        )
    ...
    def getMelbaResponse(self, message: str, person: str, stream=False) -> str:
        ...
        self.llm.loadPrompt(
            path=None, prompt=self.curPrompt, type=self.llmConfig.modelType
        )
        ...
        self.llm.reset()  
        self.logger.log(
            message=f"User: '{person}'\tMessage: '{message}'\tMelba response: '{response}'"
            f"\tEmotions: "
            f"'{emotion}'"
        )
...
class Memory:
    ...
    def accessMemories(
        ...
        response = (
            self.memoryDB.metadataQueryDB(type=type, identifier=identifier)
            if not vectorSearch
            else json.loads(
                json.dumps(
                    self.memoryDB.vectorQueryDB(
                        queries=[identifier], filter=type, nResults=1
                    )
                )
            )["documents"]
        )
        ...
        if response == "":
            self.logger.log(
                message=f"melbaToast: No '{type}' memory with identifier '{identifier}' found."
            )
    ...
    def systemPrompt(self):
        response = self.accessMemories(
            type="systemPrompt", identifier="Melba Toast", vectorSearch=True
        )[0][0]
        return (
            self.accessMemories(type="systemPrompt", identifier="generic")
            if response == ""
            else response
        )
    ...
    def updateMemory(self, type: str, identifier: str, newContent: str):
        self.memoryDB.updateOrCreateDBEntry(
            type=type, identifier=identifier, content=newContent
        )
        self.logger.log(
            message=f"melbaToast: Updated memory entry of type '{type}' "
            f"with identifier '{identifier}' and content '{newContent}'"
        )

    def saveConversation(self, person: str, conversation: str):
        lines = conversation.count("<|im_start|>")
        if lines > 4:
...
class Context:
    ...
    def situationalContext(self, person: str, message: str) -> str:
        vectorstorageresponse = self.memoryDB.vectorQueryDB(
            queries=[message], filter="information", nResults=2
        )
        context = []
        ...
        formattedcontext += (
            f"Current date: {datetime.today().strftime('%d/%m/%Y')}\n"
...
class MelbaTools:
    ...
    def characterProbability(self, frequencies: List[StrIntPair], target: str):
        targetindex = 0
        wordsum = 0
    ...
    def sentenceEntropy(self, sentence: str):
        ...
        for field in frequencies:
            charprob = self.characterProbability(
                frequencies=frequencies, target=field.string
            )
    ...
    def isSwearWord(self, word: str) -> bool:
        if self.swearWords == "":
            swearWords = (
                self.memoryDB.metadataQueryDB(type="swearwords", identifier="all")
            ).split()
    ...
    def maliciousWordsCount(self, words: List[str]) -> int:
        if self.maliciousWords == "":
            self.maliciousWords = self.memoryDB.metadataQueryDB(
                type="maliciouswords", identifier="all"
            ).split()
    ...
    def preprocessMessage(self, message: str) -> str:
        if (
            self.sentenceEntropy(sentence=message) > 1.5
            and self.maliciousWordsCount(words=message.split()) <= 0
        ):
