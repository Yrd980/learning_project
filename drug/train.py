...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train the drug review sentiment model"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--lstm_units", type=int, default=64, help="Number of LSTM units"
    )
...
def setup_logging():
    logging.basicConfig(
        ...
        handlers=[logging.StreamHandler(), logging.FileHandler("trianing.log")],
    )
...
def train(args):
    ...
    train_dataset = DrugReviewDataset(args.val_file, max_length=args.max_length)
    train_loader = create_data_loader(train_dataset, args.batch_size)

    if args.val_file:
        val_dataset = DrugReviewDataset(args.val_file, args.batch_size)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            writer.add_scalar(
                "Training/BatchAccuracy", 100.0 * correct / total, global_step
            )
            progress_bar.set_postfix(
                {"loss": total_loss / len(train_loader), "acc": 100.0 * correct / total}
            )
