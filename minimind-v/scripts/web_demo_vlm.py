...
def init_model(lm_config):
    ...
    if args.load == 0:
        ...
        model = MiniMindVLM(
            lm_config, vision_model_path="../model/vision_model/clip-vit-base-patch16"
        )
    ...
    print(
        f"VLM parameters：{sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6:.3f} millions"
    )
...
def chat(prompt, current_image_path):
    ...
    pixel_values = (
        MiniMindVLM.image2tensor(image, preprocess).to(model.device).unsqueeze(0)
    )
        ...
        with gr.Row():
                ...
                with gr.Blocks() as iface:
                    with gr.Row():
                        image_input = gr.Image(
                            type="filepath", label="select picture", height=650
                        )
                ...
                with gr.Blocks() as iface_param:
                    with gr.Row():
                        temperature_slider = gr.Slider(
                            label="Temperature", minimum=0.5, maximum=1.1, value=0.65
                        )
                        top_p_slider = gr.Slider(
                            label="Top-P", minimum=0.7, maximum=0.95, value=0.85
                        )

                    temperature_slider.change(
                        fn=update_parameters, inputs=[temperature_slider, top_p_slider]
                    )
                    top_p_slider.change(
                        fn=update_parameters, inputs=[temperature_slider, top_p_slider]
                    )
            ...
            with gr.Column(scale=6):
                ...
                def chat_with_vlm(message, history):
                    if not message:
                        yield history + [("error", "error: message can't empty")]
                        return
                    if not current_image_path:
                        yield history + [("error", "error: image can't empty")]
                        return
                ...
                submit_button.click(
                    fn=chat_with_vlm, inputs=[message_input, chatbot], outputs=chatbot
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="0: original torch parameters 1: transformers load",
    )
