...
def init_model(lm_config, device):
    ...
    print(
        f"VLM parameters：{sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6:.3f} million"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="0: origin torch weight，1: transformers load",
    )
    parser.add_argument(
        "--model_mode",
        default=1,
        type=int,
        help="0: Pretrain，1: SFT，2: SFT-multi",
    )
    ...
    if args.use_multi == 2:
        ...
        pixel_tensors = (
            torch.cat(pixel_tensors_multi, dim=0).to(args.device).unsqueeze(0)
        )
        for _ in range(10):
