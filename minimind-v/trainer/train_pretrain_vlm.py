...
def train_epoch(epoch, wandb):
    ...
    for step, (X, Y, loss_mask, pixel_values) in enumerate(train_loader):
        ...
        if (step + 1) % args.save_interval == 0 and (not ddp or dist.get_rank() == 0):
            ...
            ckp = (
                f"{args.save_dir}/pretrain_vlm_{model_config.hidden_size}{moe_path}.pth"
            )
            ...
            clean_state_dict = {
                k: v.half() for k, v in clean_state_dict.items()
            }  
...
def init_model(model_config: VLMConfig):
    ...
    model = MiniMindVLM(
        model_config, vision_model_path="../model/vision_model/clip-vit-base-patch16"
    )
    ...
    Logger(
        f"VLM train parameters：{sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6:.3f} million"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_path", type=str, default="../dataset/pretrain_data.jsonl"
    )
