...
def init_model(args):
    if args.load == 0:
        ...
        if args.lora_name != "None":
            apply_lora(model)
            load_lora(
                model, f"../{args.out_dir}/{args.lora_name}_{args.hidden_size}.pth"
            )
    else:
    ...
    print(
        f"MiniMind model parameters: {sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6:.2f}M(illion)"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="0: use origin torch weight，1: use transformers load",
    )
    parser.add_argument(
        "--model_mode",
        default=1,
        type=int,
        help="0: Pretrain，1: SFT-Chat，2: RLHF-Chat，3: Reason",
    )
