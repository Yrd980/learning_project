...
function _get_kernel() {
  ...
  [ ! -f "$ZIP_MIHOMO" ] && [ ! -f "$ZIP_CLASH" ] && {
    local arch=$(uname -m)
    _failcat "${ZIP_BASE_DIR}：not detect available kernel zip"
...
function _get_proxy_port() {
  ...
  _is_already_in_use "$MIXED_PORT" "$BIN_KERNEL_NAME" && {
    ...
    local msg="port occupy：${MIXED_PORT} 🎲 random allocate：$newPort"
...
function _get_ui_port() {
  ...
  _is_already_in_use "$UI_PORT" "$BIN_KERNEL_NAME" && {
    local newPort=$(_get_random_port)
    local msg="port occupy：${UI_PORT} 🎲 random allocate：$newPort"
...
_is_bind() {
  local port=$1
  { sudo ss -anptu || sudo netstat -anptu; } | grep ":${port}\b"
}
...
function _valid_env() {
  _is_root || _error_quit "need root or sudo privilege to execute"
  [ -n "$ZSH_VERSION" ] && [ -n "$BASH_VERSION" ] && _error_quit "only support ：bash、zsh"
...
function _valid_config() {
  [ -e "$1" ] && [ "$(wc -l <"$1")" -gt 1 ] && {
    ...
    msg=$(eval "$cmd") || {
      eval "$cmd"
      echo "$msg" | grep -qs "unsupport proxy type" && _error_quit "unsupport proxy type，please install mihomo kernel"
    }
  }
}
...
_download_clash() {
  ...
  *86*)
  ...
  aarch64)
    ...
    ;;
  *)
    _error_quit "unkonwn architecture：$arch，please manually download to ${ZIP_BASE_DIR} dirtory：https://downloads.clash.wiki/ClashPremium/"
    ;;
  esac
  ...
  echo $sha256sum "$clash_zip" | sha256sum -c ||
    _error_quit "download error：please manually download to ${ZIP_BASE_DIR} dirtory：https://downloads.clash.wiki/ClashPremium/"
}
...
function _download_config() {
  ...
  _okcat '🍃' 'download successful： check config...'
  _valid_config "$dest" || {
    _failcat '🍂' "check error：try to convert subscribe..."
    _download_convert_config "$dest" "$url" || _failcat '🍂' "subconvert error：please check log：$BIN_SUBCONVERTER_LOG"
  }
}
...
_start_convert() {
  _is_already_in_use $BIN_SUBCONVERTER_PORT 'subconverter' && {
    local newPort=$(_get_random_port)
    _failcat '🎯' "port occupy：$BIN_SUBCONVERTER_PORT 🎲 random allocate：$newPort"
  ...
  while ! _is_bind "$BIN_SUBCONVERTER_PORT" >&/dev/null; do
    sleep 1s
    local now=$(date +%s)
    [ $((now - start)) -gt 1 ] && _error_quit "subscribe convert service not start，please check log：$BIN_SUBCONVERTER_LOG"
  done
}
_stop_convert() {
